 ...
 * OnlineRoomSetup.js
 * ════════════════════════════════════════════════
 * شاشة اختيار وضع اللعب الأونلاين — قابلة لإعادة الاستخدام
 *
 * تُستخدم في: XOGameScreen, BullshitGameScreen, WordleGameScreen
 * (DrawGuessGameScreen و MafiaGameScreen و CodenamesGameScreen لها منطقها الخاص)
 *
 * الأوضاع:
 *   random  → بحث تلقائي عن منافس
 *   create  → إنشاء غرفة بكود + نسخ ومشاركة
 *   join    → إدخال كود للانضمام
 *
 * Props:
 *   gameEmoji   : string  — رمز اللعبة (مثلاً '✕○')
 *   gameTitleAr : string  — اسم اللعبة بالعربية
 *   gameTitleEn : string  — اسم اللعبة بالإنجليزية
 *   descAr      : string  — وصف قصير عربي
 *   descEn      : string  — وصف قصير إنجليزي
 *   onBack      : func    — زر الرجوع
 *   onSelect    : func(mode, code?) — اختيار الوضع
 *                 mode: 'random' | 'create' | 'join'
 *                 code: string | null (عند join فقط)
 */
    ...
    <View style={[s.container, { backgroundColor: 'transparent' }]}>
      ...
      >
        ...
        <View style={s.header}>
          ...
          {!!desc && (
            <Text style={[s.desc, { color: theme.textSecondary }]}>{desc}</Text>
          )}
        </View>
        ...
        <ThemedCard
          ...
          style={s.optionCard}
        >
          <View style={[s.optionIcon, { backgroundColor: theme.accentSoft }]}>
            <Text style={s.optionEmoji}>🌐</Text>
          </View>
          <View style={s.optionText}>
            <Text style={[s.optionTitle, { color: theme.textPrimary }]}>
              {isRTL ? 'لعب عشوائي' : 'Random Match'}
            </Text>
            <Text style={[s.optionSub, { color: theme.textSecondary }]}>
              {isRTL ? 'ابحث عن منافس تلقائياً' : 'Find an opponent automatically'}
            </Text>
          </View>
          <Text style={[s.arrow, { color: theme.textMuted }]}>{'←'}</Text>
        </ThemedCard>
        ...
        <ThemedCard
          ...
          style={s.optionCard}
        >
          <View style={[s.optionIcon, { backgroundColor: '#10b98120' }]}>
            <Text style={s.optionEmoji}>🔗</Text>
          </View>
          <View style={s.optionText}>
            <Text style={[s.optionTitle, { color: theme.textPrimary }]}>
              {isRTL ? 'إنشاء غرفة مع صديق' : 'Create Friend Room'}
            </Text>
            <Text style={[s.optionSub, { color: theme.textSecondary }]}>
              {isRTL ? 'احصل على كود وشاركه مع صديقك' : 'Get a code and share it'}
            </Text>
          </View>
          <Text style={[s.arrow, { color: theme.textMuted }]}>{'←'}</Text>
        </ThemedCard>
        ...
        {/* ── الانضمام بكود ── */}
        <ThemedCard radius={18} padding={18} style={s.joinCard}>
...
// ── شاشة الانتظار (تُعرض بعد اختيار الوضع) ─────────────────────
export function OnlineWaitingLobby({
  ...
  return (
    <View style={[wl.container, { backgroundColor: 'transparent' }]}>
      ...
      <Text style={[wl.title, { color: theme.textPrimary }]}>
        {isRTL ? 'في انتظار صديق...' : 'Waiting for friend...'}
      </Text>

      {isFriend && friendCode ? (
        <View style={wl.codeSection}>
          <Text style={[wl.codeLbl, { color: theme.textSecondary }]}>
          ...
          >
            ...
            <Text style={[wl.copyHint, { color: theme.textMuted }]}>
        ...
        <Text style={[wl.subText, { color: theme.textSecondary }]}>
